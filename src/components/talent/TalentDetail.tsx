...
export function TalentDetail({ talent: t, saved, onSave }: { talent: Talent; saved: boolean; onSave: () => void }) {
  ...
  return <>
    ...
    <div className={styles.detailEyebrow}>DEVELOPER PROFILE <span>{t.pending ? '预览档案' : '虚构示例档案'}</span></div>
    ...
    <div className={styles.profileColumns}>
      <section className={styles.profilePanel} aria-label="代表项目"><div className={styles.panelHeading}><h3><FolderGit2 size={17} />代表项目</h3><span>{t.projects?.length ?? 0} 个项目</span></div>
        {t.projects?.length ? t.projects.map(p => <div className={styles.projectRecord} key={p.name}><div><strong>{p.name}</strong><span className={styles.relationTag} data-kind={p.relationship}>{p.relationship === 'own' ? <Code2 size={11} /> : <GitPullRequest size={11} />}{p.relationship === 'own' ? '自有项目' : 'PR 贡献'}</span></div><p>{p.description}</p><small>{p.contribution}</small></div>) : <div className={styles.panelEmpty}><FolderGit2 size={22} /><p>暂未关联代表项目</p><small>后续可添加自有仓库或参与贡献的 PR。</small></div>}
      </section>
      <section className={styles.profilePanel} aria-label="信息来源"><div className={styles.panelHeading}><h3><Globe2 size={17} />信息来源</h3><span>{t.pending ? '待补充' : '示例来源'}</span></div>
        {t.sources?.length ? t.sources.map(s => { const Icon = s.kind === 'github' ? GitFork : s.kind === 'article' ? FileText : Globe2; return <div className={styles.evidenceRecord} key={s.title}><span className={styles.evidenceIcon}><Icon size={16} /></span><div><small>{s.publisher}</small><strong>{s.title}</strong><p>{s.description}</p></div></div>; }) : <div className={styles.panelEmpty}><Globe2 size={22} /><p>{t.handle ? `GitHub · @${t.handle}` : '人工收录'}</p><small>尚未关联来源，可补充 GitHub、个人网站或相关报道。</small></div>}
      </section>
    </div>
    {!t.pending && <p className={styles.evidenceDisclaimer}>项目、PR 和报道均为虚构示例，尚未关联真实链接。</p>}
