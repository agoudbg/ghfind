...
export function TalentIntake({ onClose, onAdd }: { onClose: () => void; onAdd: (talent: Talent) => void }) {
  ...
  return <Dialog open onOpenChange={open => { if (!open) onClose(); }}><DialogContent className={styles.intakeDialog}>
    ...
    <form onSubmit={next} className={styles.intakeForm}>
      <div className={styles.intakeBody}>
        ...
        {step === 3 && <><h3>这就是别人能看到的档案</h3><p>只有下方内容会带入人才库。隐藏字段不会附加到档案数据中。</p><div className={styles.publicPreview}><span className={`${styles.avatar} ${styles.sage}`}>{talent.initials}</span><div><strong>{talent.name}</strong><small>{talent.handle ? `@${talent.handle} · GitHub 资料待接入` : '手动收录'}</small></div></div><dl className={styles.publicFields}>{profileFields.filter(([key]) => talent.publicFields?.[key]).map(([key, label]) => <div key={key}><dt>{label}</dt><dd>{talent.publicFields?.[key]}</dd></div>)}</dl>{!Object.keys(talent.publicFields ?? {}).length && <div className={styles.privacyNote}><LockKeyhole size={16} /> 仅展示{method === 'github' ? ' GitHub 用户名' : '昵称'}，未公开其他个人资料。</div>}{talent.note !== '本次预览收录。' && <p>推荐语：{talent.note}</p>}<p className={styles.prototypeNote}>这是本地前端预览，尚未发布或验证身份，刷新后重置。正式收录自己的账号需要本人验证。</p></>}
      </div>
      {error && <p role="alert" className={styles.intakeError}>{error}</p>}
      <div className={styles.intakeFooter}><button type="button" className={styles.secondary} onClick={() => { setError(''); if (step) setStep(step - 1); else onClose(); }}><ArrowLeft size={14} />{step ? '上一步' : '取消'}</button><span>{step === 2 && self ? '全部选填，可直接继续' : '本次预览不向服务器提交'}</span><button className={styles.primary} type="submit">{step === 3 ? '确认加入预览' : '下一步'}<ArrowRight size={14} /></button></div>
    </form>
